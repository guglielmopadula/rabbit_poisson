...
def gaussian_kernel(f1: Tensor, f2: Tensor, degree: int = 3, gamma: Optional[float] = None, coef: float = 1.0) -> Tensor:
    """Adapted from `KID Score`_"""
    if gamma is None:
        gamma = f1.shape[1]
    kernel = torch.exp(-torch.sum(((f1.reshape(f1.shape[0],1,f1.shape[1]).repeat(1,f1.shape[0],1) - torch.transpose(f2.reshape(f2.shape[0],1,f2.shape[1]).repeat(1,f2.shape[0],1),0,1))/gamma)**2,axis=2))
    return kernel
...
def gaussian_mmd(
    ...
    """Adapted from `KID Score`_"""
    k_11 = gaussian_kernel(f_real, f_real, degree, gamma, coef)
    k_22 = gaussian_kernel(f_fake, f_fake, degree, gamma, coef)
    k_12 = gaussian_kernel(f_real, f_fake, degree, gamma, coef)
...
class KernelInceptionDistance(Metric):
    ...
    def compute(self) -> Tuple[Tensor, Tensor]:
        ...
        for _ in range(self.subsets):
            ...
            o = gaussian_mmd(f_real, f_fake, self.degree, self.gamma, self.coef)
